"""Deployment prompt embedding cache for low-memory WBWAM inference."""
...
from bridge.common.config import read_deployment_config
...
def deployment_prompt_embedding_spec(
    ...
    *,
    ...
    deployment_cfg = read_deployment_config(config_path)
    policy_cfg = dict(policy_config if policy_config is not None else deployment_cfg.get("policy") or {})

    _, training_cfg = load_training_config(policy_cfg)
    model_cfg = training_cfg.model
    data_cfg = training_cfg.data
    prompt = format_deployment_prompt(training_cfg, policy_cfg, task_prompt)
    ...
    checkpoint_path = _resolve_checkpoint_path(policy_cfg["checkpoint_path"])
    ...
    explicit_path = _resolve_path(policy_cfg.get("prompt_embedding_path"))
    output_path = explicit_path or cache_dir / f"{prompt_sha256}.t5_len{context_len}.{cache_suffix}.pt"

    return PromptEmbeddingSpec(
        prompt=prompt,
        prompt_sha256=prompt_sha256,
        output_path=output_path,
        wbwam_root=TRAINING_ROOT,
...
def _load_cache_payload(path: Path, *, expected: PromptEmbeddingSpec | None = None) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict) or "context" not in payload or "mask" not in payload:
        raise ValueError(f"invalid WBWAM prompt embedding cache: {path}")
    ...
    if expected is not None:
        ...
        if payload.get("prompt") != expected.prompt:
            raise ValueError(f"cached prompt text does not match requested prompt: {path}")
...
def ensure_prompt_embedding(
    ...
    *,
    ...
    spec = deployment_prompt_embedding_spec(deployment_config_path, task_prompt, policy_config=policy_config)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--policy-json", default="null", help=argparse.SUPPRESS)
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    spec = deployment_prompt_embedding_spec(args.config, args.prompt, policy_config=json.loads(args.policy_json))
    if spec.output_path.exists():
        _load_cache_payload(spec.output_path, expected=spec)
