...
class WBFlatCodec:
    ...
    def __init__(self, data_config: Any, dataset_stats: dict[str, Any]) -> None:
        ...
        self.model_dim_indices = _shared_model_dim_indices(
        ...
        if self.action_representation == "absolute" and self.relative_joint_ranges:
            raise ValueError("absolute WB data config cannot define relative_joint_ranges")
        if self.action_representation not in {"absolute", "relative_joint"}:
        ...
        if self.model_dim_indices is None:
        ...
        if self.model_dim_indices is not None:
            model_widths = {
                "proprio_dim": self.proprio_dim,
                "action_target_dim": self.action_target_dim,
            }
            for name, expected in model_widths.items():
        ...
        if "model_dim_indices" in metadata:
            stats_model_dim_indices = _shared_model_dim_indices(
                metadata["model_dim_indices"],
                state_dim=self.state_dim,
                action_dim=self.action_dim,
                proprio_dim=self.proprio_dim,
                action_target_dim=self.action_target_dim,
        ...
        if self.action_representation == "relative_joint" or self.model_dim_indices is not None:
    ...
    def denormalize_action(self, action: Any) -> np.ndarray:
        ...
        if value.ndim != 3:
            raise ValueError(f"expected WBWAM action [B,T,D], got {tuple(value.shape)}")
        if self.model_dim_indices is None:
            ...
            semantic = value[..., list(self.model_dim_indices)]
