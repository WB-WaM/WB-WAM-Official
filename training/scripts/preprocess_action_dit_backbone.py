...
from wbwam.models.wan22.action_dit import ActionDiT
from wbwam.models.wan22.helpers.loader import load_wan22_ti2v_5b_components
...
def _load_model_config(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if "video_dit_config" not in cfg or "action_dit_config" not in cfg:
        raise ValueError(f"`{path}` must contain both `video_dit_config` and `action_dit_config` at top level.")
    ...
    for key in ["action_dim"]:
        ...
        if val is None or _is_unresolved_interpolation(val):
            print(f"[WARN] `action_dit_config.{key}` is unresolved or missing; defaulting to 7 for preprocessing.")
            action_cfg[key] = 7
...
def main() -> None:
    ...
    parser.add_argument(
        "--model-config",
        required=True,
        help="Path to model yaml, e.g. configs/model/wbwam_optional_idm.yaml",
    )
