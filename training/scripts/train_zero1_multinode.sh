...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/train_zero1_multinode.sh [hydra_overrides...]
...
Examples:
  ...
  bash scripts/train_zero1_multinode.sh wb_task=midtrain

  # Hydra overrides only:
  RUN_ID=exp123 bash scripts/train_zero1_multinode.sh wb_task=midtrain
EOF
}
