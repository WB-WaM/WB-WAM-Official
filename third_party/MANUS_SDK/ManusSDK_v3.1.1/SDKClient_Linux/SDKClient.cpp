...
void SDKClient::OnRawSkeletonStreamCallback(const SkeletonStreamInfo* const p_RawSkeletonStreamInfo)
{
	...
	if (s_Instance)
	{
		...
		for (uint32_t i = 0; i < p_RawSkeletonStreamInfo->skeletonsCount; i++)
		{
			...
			std::vector<NodeInfo> node_info(t_NxtClientRawSkeleton->skeletons[i].info.nodesCount);
			const SDKReturnCode t_NodeInfoResult = CoreSdk_GetRawSkeletonNodeInfoArray(
				t_NxtClientRawSkeleton->skeletons[i].info.gloveId,
				node_info.data(),
			...
			std::vector<double> parent_ids;
			std::vector<double> chain_types;
			std::vector<double> finger_joint_types;
			for (uint32_t j = 0; j < t_NxtClientRawSkeleton->skeletons[i].info.nodesCount; j++){
				...
				if (t_NodeInfoResult == SDKReturnCode::SDKReturnCode_Success)
				{
					node_ids.push_back(static_cast<double>(node_info[j].nodeId));
					parent_ids.push_back(static_cast<double>(node_info[j].parentId));
					chain_types.push_back(static_cast<double>(node_info[j].chainType));
					finger_joint_types.push_back(static_cast<double>(node_info[j].fingerJointType));
				}
				else
				{
					node_ids.push_back(0.0);
					parent_ids.push_back(0.0);
					chain_types.push_back(static_cast<double>(ChainType_Invalid));
					finger_joint_types.push_back(static_cast<double>(FingerJointType_Invalid));
				}
			}
			{
				...
				output_map[glove_id +"_node_count"] = {static_cast<double>(t_NxtClientRawSkeleton->skeletons[i].info.nodesCount)};
				output_map[glove_id +"_node_id"] = node_ids;
				output_map[glove_id +"_parent_id"] = parent_ids;
				output_map[glove_id +"_chain_type"] = chain_types;
				output_map[glove_id +"_joint_type"] = finger_joint_types;
			}
		}
...
void SDKClient::OnLandscapeCallback(const Landscape* const p_Landscape)
{
	...
	{
		std::lock_guard<std::mutex> t_OutputLock(output_map_mutex);
		output_map["glove_count"] = {static_cast<double>(t_Landscape->gloveDevices.gloveCount)};
		output_map.erase("left_glove_id");
		output_map.erase("right_glove_id");
		for (size_t i = 0; i < t_Landscape->gloveDevices.gloveCount; i++)
		{
			const GloveLandscapeData t_GloveData = t_Landscape->gloveDevices.gloves[i];
			if (t_GloveData.side == Side::Side_Left && output_map.find("left_glove_id") == output_map.end())
			{
				output_map["left_glove_id"] = {static_cast<double>(t_GloveData.id)};
			}
			else if (t_GloveData.side == Side::Side_Right && output_map.find("right_glove_id") == output_map.end())
			{
				output_map["right_glove_id"] = {static_cast<double>(t_GloveData.id)};
			}
		}
	}
