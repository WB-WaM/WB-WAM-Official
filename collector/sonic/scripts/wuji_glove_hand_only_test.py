#!/usr/bin/env python3
"""Hand-only Wuji Glove -> Wuji Hand command publisher."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Publish only Wuji hand qpos from Wuji Glove skeleton streams.",
    )
...
def _confirm(args: argparse.Namespace) -> None:
    if args.yes:
        return
    print("Wuji glove hand-only test will command robot hands.")
    print(f"  publish_bind: {args.bind}")
    print(f"  left_glove:   {args.left_address}")
    print(f"  right_glove:  {args.right_address}")
    print(f"  stale_s:      {args.stale_s}")
...
def _payload(
    *,
    ...
    return {
        ...
        "left_wuji_qpos": np.asarray(left_qpos, dtype=np.float32).reshape(20),
        "right_wuji_qpos": np.asarray(right_qpos, dtype=np.float32).reshape(20),
...
def main() -> int:
    ...
    if left_retargeter is None or right_retargeter is None:
        raise RuntimeError(
            "failed to initialize Wuji glove retargeters; check adaptive_analytical_wuji_glove_{left,right}.yaml"
        )
    ...
    print("[OK] Hand-only publisher started")
    ...
    left_qpos = np.zeros((20,), dtype=np.float32)
    right_qpos = np.zeros((20,), dtype=np.float32)
    ...
    try:
        while not stop:
            ...
            (
                ...
                right_keypoints,
                right_fresh,
            ) = provider.latest_keypoints(last_left_keypoints, last_right_keypoints)
            if left_fresh:
    ...
    print("[OK] Hand-only publisher stopped")
    return 0
