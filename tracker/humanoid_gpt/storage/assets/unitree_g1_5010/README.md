...
This asset is specifically mode 15. A 5010 wrist motor by itself is not enough
to identify the complete G1 variant: current Unitree descriptions also include
other 5010 machine modes with different hip ratios and/or a locked waist. For
real deployment, verify `Device -> Data -> Robot -> Machine Type` in the
Unitree App. Asset selection must remain separate from the low-level transport
header: `LowCmd.mode_machine` should echo live `LowState.mode_machine`; never
force 15 solely because this asset is loaded.
