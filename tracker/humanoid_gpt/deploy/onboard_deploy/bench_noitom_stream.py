    ...
    *,
    ...
    while True:
        ...
        if mocap_ts > 0.0 and mocap_ts != last_ts:
            ...
            if last_arrival_monotonic > 0.0:
                arrival_dt.append(now_monotonic - last_arrival_monotonic)
            lag_ms.append(max(0.0, now_monotonic - mocap_ts) * 1e3)
            updates += 1
            last_ts = mocap_ts
            last_arrival_monotonic = now_monotonic

        stale_for = now_monotonic - last_arrival_monotonic
        if (
            last_arrival_monotonic > 0.0
            and stale_for > stale_warn_sec
            and now_monotonic - last_stale_report_t > stale_warn_sec
        ):
            stale_events += 1
            last_stale_report_t = now_monotonic
