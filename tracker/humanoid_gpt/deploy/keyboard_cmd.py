            ...
            for name, d in self._dims.items():
                if isinstance(d, CmdDim):
                    ...
                    if hi ^ lo:
                        ...
                        self._vals[name] = _clamp(
                            float(self._vals[name]) + inc, d.low, d.high
                        )
                    else:
...
class CommandWindow:
    ...
    def __init__(
        self, commander: KeyboardCommander, *, width: int = 720, fps: int = 60
    ):
        if not pygame.get_init():
            pygame.init()
        ...
        self.font_title = pygame.font.SysFont(
            "Segoe UI,Helvetica,Arial,sans-serif", 28, bold=True
        )
        ...
        self.font_mode = pygame.font.SysFont(
            "Helvetica,Arial,sans-serif", 15, bold=True
        )
        ...
        self._mode_btns = [
            d
            for d in commander._dims.values()
    ...
    def draw_frame(self):
        ...
        for d in self._axes:
            ...
            pygame.draw.line(
                ...
                _BORDER_LIT,
                (mid_x, iy + 10),
                (mid_x, iy + 12 + self.BAR_H + 6),
                1,
            )

            # Value text
            val_color = (
                _BAR_POS if v > 0.001 else (_BAR_NEG if v < -0.001 else _TEXT_DIM)
            )
            ...
            key_surf = self.font_key.render(
                f"{d.high_key.upper()} / {d.low_key.upper()}", True, _TEXT_DIM
            )
        ...
        if self._action_btns:
            ...
            for b in self._action_btns:
                ...
                lbl = self.font_label.render(
                    b.name, True, _TEXT if state else _TEXT_DIM
                )
...
class KeyboardCmdPad:
    ...
    def __init__(
        ...
        *,
        ...
        self._shm = ctx.Array("f", n, lock=True)
        self._stop = ctx.Value("i", 0)
        ...
        for d in dims:
            if isinstance(d, CmdDim):
                ...
                dim_specs.append(
                    ("btn", {"name": d.name, "key": d.key, "init": d.init})
                )
    ...
    def is_alive(self) -> bool:
        """Whether the GUI is still available to deliver stop commands."""
        return self._stop.value == 0 and self._proc.is_alive()
...
class DeployKeyboardCMD:
        ...
        self._cmd_pad = KeyboardCmdPad(
            dims=[
                CmdDim(
                    ...
                    "s",
                    "w",
                    low=-KEYBOARD_MAX_SPEED,
                    high=KEYBOARD_MAX_SPEED,
                    delta=0.05,
                ),
                CmdDim(
                    "LinVelY",
    ...
    def step_command(self) -> HighCommand:
        if not self._cmd_pad.is_alive:
            return HighCommand(mode=self.mode, kill=True)
    ...
    def set_mode(self, mode: int) -> None:
        """Persist a mode selected by an input source other than the keyboard."""
        mode = int(mode)
        if not 0 <= mode < self.num_mode:
            raise ValueError(f"mode must be in [0, {self.num_mode}), got {mode}")
        self.mode = mode
