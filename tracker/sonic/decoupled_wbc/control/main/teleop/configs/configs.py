...
WB_WAM_ROOT = Path(__file__).resolve().parents[7]
...
class BaseConfig(ArgsConfigTemplate):
    ...
    wbc_model_path: str = (
        f"{WB_WAM_ROOT}/checkpoints/tracker/sonic/decoupled_wbc/policy/GR00T-WholeBodyControl-Balance.onnx,"
        f"{WB_WAM_ROOT}/checkpoints/tracker/sonic/decoupled_wbc/policy/GR00T-WholeBodyControl-Walk.onnx"
    )
    """Path to WBC model files. Absolute paths are supported."""
