...
show_usage() {
    ...
    echo "  --build                 Build before deployment (default: off)"
    ...
    echo "  --v1-replay-source-hz HZ"
    echo "                          Enable packet-driven v1 replay (supported: 20 or 50)"
    echo "  --replay-target TARGET  Require replay payload target: sim or real"
    echo "  --disable-teleop-safety-gate"
    echo "                          Disable streamed teleop jump protection (default: enabled)"
    echo "  --hand-backend BACKEND  Set hand backend: local_wuji or remote_wuji_proxy (default: local_wuji)"
    echo "  --left-wuji-serial S    Set left WujiHand serial"
    echo "  --right-wuji-serial S   Set right WujiHand serial"
    echo ""
    ...
    echo "  $0 --build real # Build first, then deploy"
...
CHECKPOINT_DEFAULT="../../../checkpoints/tracker/sonic/policy/release/model"
OBS_CONFIG_DEFAULT="../../../checkpoints/tracker/sonic/policy/release/observation_config.yaml"
PLANNER_DEFAULT="../../../checkpoints/tracker/sonic/planner/target_vel/V2/planner_sonic.onnx"
...
LEFT_WUJI_SERIAL_DEFAULT="307735743434"
RIGHT_WUJI_SERIAL_DEFAULT="3365364A3434"
HAND_BACKEND_DEFAULT="local_wuji"
BUILD_BEFORE_RUN=false
DISABLE_TELEOP_SAFETY_GATE=false
V1_REPLAY_SOURCE_HZ=""
REPLAY_TARGET=""
...
LEFT_WUJI_SERIAL="$LEFT_WUJI_SERIAL_DEFAULT"
RIGHT_WUJI_SERIAL="$RIGHT_WUJI_SERIAL_DEFAULT"
HAND_BACKEND="$HAND_BACKEND_DEFAULT"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --zmq-port)
            if [[ $# -lt 2 || -z "${2:-}" ]]; then
                echo -e "${RED}Error: --zmq-port requires a value${NC}" >&2
                exit 1
            fi
            if [[ ! "$2" =~ ^[1-9][0-9]{0,4}$ ]] || (( 10#$2 > 65535 )); then
                echo -e "${RED}Error: --zmq-port must be an integer in 1..65535${NC}" >&2
                exit 1
            fi
            ZMQ_PORT="$2"
            shift 2
        ...
        --v1-replay-source-hz)
            if [[ $# -lt 2 || -z "$2" ]]; then
                echo -e "${RED}Error: --v1-replay-source-hz requires a value${NC}" >&2
                exit 1
            fi
            if [[ "$2" != "20" && "$2" != "50" ]]; then
                echo -e "${RED}Error: --v1-replay-source-hz must be 20 or 50${NC}" >&2
                exit 1
            fi
            V1_REPLAY_SOURCE_HZ="$2"
            shift 2
            ;;
        --replay-target)
            if [[ $# -lt 2 || -z "${2:-}" ]]; then
                echo -e "${RED}Error: --replay-target requires a value${NC}" >&2
                exit 1
            fi
            if [[ "$2" != "sim" && "$2" != "real" ]]; then
                echo -e "${RED}Error: --replay-target must be sim or real${NC}" >&2
                exit 1
            fi
            REPLAY_TARGET="$2"
            shift 2
            ;;
        --disable-teleop-safety-gate)
            DISABLE_TELEOP_SAFETY_GATE=true
            shift
            ;;
        --hand-backend)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --hand-backend requires a backend argument${NC}" >&2
                exit 1
            fi
            if [[ "$2" != "local_wuji" && "$2" != "remote_wuji_proxy" ]]; then
                echo -e "${RED}Error: --hand-backend must be local_wuji or remote_wuji_proxy${NC}" >&2
                exit 1
            fi
            HAND_BACKEND="$2"
            shift 2
        ...
        --left-wuji-serial)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --left-wuji-serial requires a serial argument${NC}" >&2
                exit 1
            fi
            LEFT_WUJI_SERIAL="$2"
            shift 2
        ...
        --right-wuji-serial)
            if [[ -z "$2" ]]; then
                echo -e "${RED}Error: --right-wuji-serial requires a serial argument${NC}" >&2
                exit 1
            fi
            RIGHT_WUJI_SERIAL="$2"
            shift 2
...
if [[ "$DISABLE_TELEOP_SAFETY_GATE" == true ]]; then
    EXTRA_ARGS+=(--disable-teleop-safety-gate)
fi
EXTRA_ARGS+=(--zmq-port "$ZMQ_PORT")
if [[ -n "$V1_REPLAY_SOURCE_HZ" ]]; then
    EXTRA_ARGS+=(--v1-replay-source-hz "$V1_REPLAY_SOURCE_HZ")
fi
...
if [ "$BUILD_BEFORE_RUN" = true ]; then
    ...
    echo "Skipping build (pass --build to rebuild before deployment)"
    if [ ! -x "target/release/g1_deploy_onnx_ref" ]; then
        echo -e "${RED}❌ Missing executable: target/release/g1_deploy_onnx_ref${NC}"
        echo "   Run 'just build' first, or rerun this script with --build"
        exit 1
    fi
fi
...
echo -e "  Hand Backend:       ${GREEN}$HAND_BACKEND${NC}"
echo -e "  Left Wuji Serial:   ${GREEN}$LEFT_WUJI_SERIAL${NC}"
echo -e "  Right Wuji Serial:  ${GREEN}$RIGHT_WUJI_SERIAL${NC}"
if [[ -n "$V1_REPLAY_SOURCE_HZ" ]]; then
echo -e "  V1 Replay Source:   ${GREEN}${V1_REPLAY_SOURCE_HZ} Hz${NC}"
fi
if [[ -n "$REPLAY_TARGET" ]]; then
echo -e "  Replay Target:      ${GREEN}${REPLAY_TARGET}${NC}"
fi
...
echo -e "${BLUE}    --hand-backend $HAND_BACKEND \\${NC}"
echo -e "${BLUE}    --left-wuji-serial $LEFT_WUJI_SERIAL \\${NC}"
echo -e "${BLUE}    --right-wuji-serial $RIGHT_WUJI_SERIAL${NC}"
if [[ ${#EXTRA_ARGS[@]} -gt 0 ]]; then
