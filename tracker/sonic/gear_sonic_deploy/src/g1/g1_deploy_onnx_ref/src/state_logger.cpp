...
uint64_t StateLogger::LogFullState(const std::array<double, 4>& base_quat,
  ...
  e.timestamp_monotonic =
      timestamp_monotonic == std::chrono::steady_clock::time_point{}
...
bool StateLogger::LogPostReplayMetadata(
  int64_t replay_route,
  int64_t replay_stream_epoch,
  int64_t replay_source_frame,
  bool encoder_cache_refresh,
  const std::array<int64_t, 10>& encoder_input_indices,
  double last_pose_rx_age_ms,
  int64_t replay_fault_code,
  bool safety_fall_detected,
  int64_t safety_reset_count,
  int64_t safety_joint_position_limit_violation_count,
  int64_t safety_joint_velocity_limit_violation_count,
  int64_t safety_command_limit_violation_count,
  int64_t hand_frame_index
) {
  std::lock_guard<std::mutex> lock(ring_mutex_);
  if (size_ == 0) {
    std::cerr << "[StateLogger ERROR] LogPostReplayMetadata called before LogFullState." << std::endl;
    return false;
  }

  const size_t newest_idx = (start_ + size_ - 1) % capacity_;
  Entry& newest = ring_[newest_idx];
  if (newest.has_replay_metadata) {
    std::cerr << "[StateLogger ERROR] LogPostReplayMetadata called more than once for entry "
              << newest.index << "." << std::endl;
    return false;
  }

  newest.replay_route = replay_route;
  newest.replay_stream_epoch = replay_stream_epoch;
  newest.replay_source_frame = replay_source_frame;
  newest.hand_frame_index = hand_frame_index;
  newest.encoder_cache_refresh = encoder_cache_refresh;
  newest.encoder_input_indices = encoder_input_indices;
  newest.last_pose_rx_age_ms = last_pose_rx_age_ms;
  newest.replay_fault_code = replay_fault_code;
  newest.safety_telemetry_version = 1;
  newest.safety_fall_detected = safety_fall_detected;
  newest.safety_reset_count = safety_reset_count;
  newest.safety_joint_position_limit_violation_count =
      safety_joint_position_limit_violation_count;
  newest.safety_joint_velocity_limit_violation_count =
      safety_joint_velocity_limit_violation_count;
  newest.safety_command_limit_violation_count =
      safety_command_limit_violation_count;
  newest.has_replay_metadata = true;
  return true;
}

bool StateLogger::LogPostMotorCommand(const MotorCommand& motor_command) {
  std::lock_guard<std::mutex> lock(ring_mutex_);
  if (size_ == 0) {
    std::cerr << "[StateLogger ERROR] LogPostMotorCommand called before LogFullState." << std::endl;
    return false;
  }

  const size_t newest_idx = (start_ + size_ - 1) % capacity_;
  Entry& newest = ring_[newest_idx];
  if (newest.has_motor_command) {
    std::cerr << "[StateLogger ERROR] LogPostMotorCommand called more than once for entry "
              << newest.index << "." << std::endl;
    return false;
  }

  newest.q_target = motor_command.q_target;
  newest.dq_target = motor_command.dq_target;
  newest.kp = motor_command.kp;
  newest.kd = motor_command.kd;
  newest.tau_ff = motor_command.tau_ff;
  newest.has_motor_command = true;
  return true;
}
...
Entry StateLogger::makeZeroEntry_() const {
  ...
  e.left_hand_q.assign(kWujiHandDoF, 0.0);
  e.left_hand_dq.assign(kWujiHandDoF, 0.0);
  e.right_hand_q.assign(kWujiHandDoF, 0.0);
  e.right_hand_dq.assign(kWujiHandDoF, 0.0);
  e.last_left_hand_action.assign(kWujiHandDoF, 0.0);
  e.last_right_hand_action.assign(kWujiHandDoF, 0.0);
