...
#include "../include/input_interface/replay_payload_safety.hpp"
...
#include "../include/output_interface/replay_terminal_telemetry.hpp"
...
#include "../include/output_interface/zmq_state_action_output_handler.hpp"
...
#include "../include/wuji_hands.hpp"
 ...
 *  - Dex3Hands / WujiHands (hand controller backends)
...
class G1Deploy {
  private:
    ...
    WujiHandJointArray left_hand_joint_buffer_ = kDefaultWujiHandPose;
    WujiHandJointArray right_hand_joint_buffer_ = kDefaultWujiHandPose;
    bool has_upper_body_data_ = false;
    std::array<double, 17> upper_body_joint_positions_buffer_;
    std::int64_t hand_frame_index_buffer_ = -1;
    std::array<double, 17> upper_body_joint_velocities_buffer_;
    std::vector<double> token_state_data_;  // Token buffer (size from config)
    double v1_replay_source_hz_ = 0.0;
    bool disable_replay_safety_ = false;
    bool replay_encoder_cache_valid_ = false;
    sonic_replay::EncoderCacheKey replay_encoder_cache_key_{};
    const MotionSequence* replay_encoder_cache_motion_ = nullptr;
    int replay_encoder_cached_reserve_ = 0;
    bool replay_encoder_cache_refresh_this_tick_ = false;
    bool replay_safety_fall_detected_ = false;
    std::int64_t replay_safety_reset_count_ = 0;
    std::int64_t replay_joint_position_violation_count_ = 0;
    std::int64_t replay_joint_velocity_violation_count_ = 0;
    std::int64_t replay_command_limit_violation_count_ = 0;
    bool replay_safety_stop_requested_this_tick_ = false;
    sonic_replay::ReplaySafetyEpochGate replay_safety_epoch_gate_;
#if ENABLE_VLA_SONIC_BRIDGE
    bool has_decoder_only_token_state_ = false;  // True when decoder-only mode has a valid token to hold.
#endif
    ...
    // Hand managers.  In remote_wuji_proxy mode deploy keeps hand actions in the
    // state/action stream, but the USB hardware is driven by a robot-side proxy.
    Dex3Hands dex3_hands_;
    WujiHands wuji_hands_;
    bool use_wuji_hands_ = true;
    bool remote_wuji_proxy_ = false;
    std::string hand_backend_;
    std::string left_wuji_serial_;
    std::string right_wuji_serial_;
    ...
    WujiHandJointArray last_left_hand_action;
    WujiHandJointArray last_right_hand_action;
    ...
    sonic_replay::TerminalReplayPublishGate terminal_replay_publish_gate_;
    ...
    sonic_replay::ReplayStreamInfo GetReplayStreamInfo() const {
      return input_interface_ ? input_interface_->GetReplayStreamInfo()
                              : sonic_replay::ReplayStreamInfo{};
    }

    bool IsV1ReplayActive() const {
      const auto info = GetReplayStreamInfo();
      return info.enabled && info.source_hz > 0.0 && current_motion_ &&
             current_motion_->name == "streamed";
    }

    bool IsV4ReplayActive() const {
      return sonic_replay::IsDecoderOnlyV4Replay(GetReplayStreamInfo());
    }

    bool PublishTerminalReplayTelemetryOnce() noexcept {
      const auto replay_info = GetReplayStreamInfo();
      if (!replay_info.faulted) {
        return false;
      }
      if (!terminal_replay_publish_gate_.TryBegin()) {
        return terminal_replay_publish_gate_.Published();
      }

      try {
        double last_pose_rx_age_ms = -1.0;
        const auto last_pose_update = input_interface_->GetLastUpdateTime();
        if (last_pose_update.has_value()) {
          last_pose_rx_age_ms = std::max(
              0.0,
              std::chrono::duration<double, std::milli>(
                  std::chrono::steady_clock::now() - *last_pose_update)
                  .count());
        }
        ...
        for (auto& output_interface : output_interfaces_) {
          ...
          StateLogger::Entry terminal_entry;
          if (!output_interface->GetLastPublishedStateEntry(terminal_entry)) {
            continue;
          }
          if (!sonic_replay::MaterializeTerminalReplayFault(
                  terminal_entry, replay_info, last_pose_rx_age_ms)) {
            continue;
          }
          if (output_interface->PublishReplayEntry(terminal_entry)) {
            terminal_replay_publish_gate_.Complete();
            std::cerr
                << "[ReplaySafety] Published terminal replay fault telemetry "
                << "for source frame " << replay_info.source_frame
                << std::endl;
            return true;
          }
        }
      } catch (const std::exception& error) {
        std::cerr << "[ReplaySafety] Terminal telemetry publish failed: "
                  << error.what() << std::endl;
      } catch (...) {
        std::cerr
            << "[ReplaySafety] Terminal telemetry publish failed unexpectedly"
            << std::endl;
      }

      terminal_replay_publish_gate_.Retry();
      return false;
    }


    int ResolveMotionFrameOffset(int observation_index, int legacy_step) const {
      if (!IsV1ReplayActive()) {
        return observation_index * legacy_step;
      }
      return sonic_replay::StreamedReplayState::ResolveObservationFrameOffset(
          observation_index, legacy_step, v1_replay_source_hz_, 1.0 / control_dt_);
    }

    void ReserveMotionObservationRows(int observation_count, int legacy_step) {
      if (observation_count <= 0) {
        return;
      }
      saved_frame_for_observation_window_ = std::max(
          saved_frame_for_observation_window_,
          ResolveMotionFrameOffset(observation_count - 1, legacy_step) + 1);
    }
    ...
    bool GatherMotionRootZPositionMultiFrame(std::vector<double>& target_buffer, size_t offset, int num_frames = 5, int step_size = 5) {
      ...
      ReserveMotionObservationRows(num_frames, step_size);
      ...
      for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
        ...
        if (operator_state.play) {
          target_frame += ResolveMotionFrameOffset(frame_idx, step_size);
    ...
    bool GatherMotionJointVelocitiesMultiFrame(std::vector<double>& target_buffer, size_t offset, int num_frames = 5, int step_size = 5, std::vector<int> joint_indexes = {}) {
      ...
      for (int frame_idx = 0; frame_idx < num_frames; frame_idx++) {
        ...
        int target_frame = static_cast<int>(current_frame_) +
                           ResolveMotionFrameOffset(frame_idx, step_size);
    ...
    bool GatherTokenState(std::vector<double>& target_buffer, size_t offset) {
      ...
      const auto replay_info = GetReplayStreamInfo();
      const bool replay_active = replay_info.enabled && replay_info.source_hz > 0.0 &&
                                 current_motion_ &&
                                 current_motion_->name == "streamed";
      const auto heading_generation =
          heading_state_buffer_.GetDataWithTime().timestamp.time_since_epoch().count();
      const int encoder_mode = current_motion_ ? current_motion_->GetEncodeMode() : -999;
      const sonic_replay::EncoderCacheKey cache_key{
          replay_info.stream_epoch, replay_info.generation,
          replay_info.source_frame, encoder_mode, operator_state.play,
          heading_generation};
      if (replay_active && !reinitialize_heading_ && replay_encoder_cache_valid_ &&
          replay_encoder_cache_key_ == cache_key &&
          replay_encoder_cache_motion_ == current_motion_.get()) {
        std::copy(
            token_state_data_.begin(), token_state_data_.end(),
            target_buffer.begin() + offset);
        saved_frame_for_observation_window_ = std::max(
            saved_frame_for_observation_window_, replay_encoder_cached_reserve_);
        return true;
      }
      if (!replay_active) {
        // The default path deliberately recomputes the encoder every control
        // tick. Replay caching is opt-in and never leaks into legacy playback.
        replay_encoder_cache_valid_ = false;
      }
      ...
      if (replay_active) {
        replay_encoder_cache_refresh_this_tick_ = true;
        replay_encoder_cache_valid_ = true;
        replay_encoder_cache_key_ = sonic_replay::EncoderCacheKey{
            replay_info.stream_epoch, replay_info.generation,
            replay_info.source_frame, encoder_mode, operator_state.play,
            heading_state_buffer_.GetDataWithTime().timestamp.time_since_epoch().count()};
        replay_encoder_cache_motion_ = current_motion_.get();
        replay_encoder_cached_reserve_ = std::max(
            saved_frame_for_observation_window_,
            sonic_replay::StreamedReplayState::RequiredRows(
                10, 5, v1_replay_source_hz_, 1.0 / control_dt_));
      }
      return true;
    }
  ...
  public:
    ...
    G1Deploy(
      ...
      int collector_zmq_out_port = 5558,
      std::string collector_zmq_out_topic = "robot_state_action",
      bool enable_motion_recording = false,
      std::array<double, 3> initial_compliance = {0.05, 0.05, 0.0},
      double initial_max_close_ratio = 1.0,
      bool disable_teleop_safety_gate = false,
      std::string left_wuji_serial = "307735743434",
      std::string right_wuji_serial = "3365364A3434",
      std::string hand_backend = "local_wuji",
      double v1_replay_source_hz = 0.0,
      std::optional<bool> replay_target_real = std::nullopt,
      bool disable_replay_safety = false)
      : time_(0.0),
        publish_dt_(0.002),
        control_dt_(0.02),
        planner_dt_(0.1),
        ...
        v1_replay_source_hz_(v1_replay_source_hz),
        disable_replay_safety_(disable_replay_safety),
        hand_backend_(std::move(hand_backend)),
        left_wuji_serial_(std::move(left_wuji_serial)),
        right_wuji_serial_(std::move(right_wuji_serial)),
      ...
      if (hand_backend_ != "local_wuji" && hand_backend_ != "remote_wuji_proxy") {
        throw std::invalid_argument("Unsupported hand backend: " + hand_backend_);
      }
      remote_wuji_proxy_ = (hand_backend_ == "remote_wuji_proxy");
      use_wuji_hands_ = (hand_backend_ == "local_wuji");

      if (use_wuji_hands_) {
        std::cout << "[HandBackend] local_wuji: deploy will command local Wuji USB devices" << std::endl;
        wuji_hands_.initialize(left_wuji_serial_, right_wuji_serial_);
      } else if (remote_wuji_proxy_) {
        std::cout << "[HandBackend] remote_wuji_proxy: deploy will not open local hand USB devices" << std::endl;
      } else {
        // Retain the legacy Dex3 code path in source, but default deployment uses WujiHand.
        dex3_hands_.initialize("");
      }

      audio_thread_ = std::make_unique<AudioThread>();

      TeleopSafetyFilter::SetGlobalEnabled(!disable_teleop_safety_gate);
      std::cout << "[INFO] Teleop safety gate: "
                << (TeleopSafetyFilter::IsGlobalEnabled() ? "enabled" : "disabled")
                << std::endl;
      std::cout << "[INFO] Replay measured/command safety: "
                << (disable_replay_safety_ ? "disabled" : "enabled")
                << std::endl;
...
#if ENABLE_VLA_SONIC_BRIDGE
        if (encoder_file_path.empty() && encoder_config_.dimension == 64) {
          static constexpr std::array<double, 64> kLatentInitialMotionToken = {
          ...
          std::copy(
              kLatentInitialMotionToken.begin(),
              kLatentInitialMotionToken.end(),
              token_state_data_.begin()
          );
          has_decoder_only_token_state_ = true;
          std::cout << "✓ Decoder-only initial token loaded for v4/planner startup" << std::endl;
        }
#endif
      }
      ...
      if (input_type == "gamepad") {
        ...
        input_interface_ = std::make_unique<GamepadManager>(
          zmq_host, zmq_port, zmq_topic, zmq_conflate, zmq_verbose,
          v1_replay_source_hz_, replay_target_real);
        ...
        input_interface_ = std::make_unique<ZMQManager>(
          ...
          zmq_verbose, v1_replay_source_hz_, replay_target_real
        );
      ...
      if (input_interface_) {
        ...
        if (!use_wuji_hands_ && !remote_wuji_proxy_) {
      ...
      if (create_zmq) {
        ...
        output_interfaces_.push_back(
            std::make_unique<ZMQStateActionOutputHandler>(
                *state_logger_, collector_zmq_out_port, collector_zmq_out_topic
            )
        );
        std::cout << "Initialized collector ZMQ state/action output interface" << std::endl;
      }
    ...
    void Stop() {
      ...
      (void)PublishTerminalReplayTelemetryOnce();
     ...
     * transitions to WAIT_FOR_CONTROL and the hand controller is reset to open pose.
    ...
    bool InitControl() {
      ...
      if (time_ < duration_) {
        ...
        if (use_wuji_hands_) {
          wuji_hands_.setOpenPose(true);
          wuji_hands_.setOpenPose(false);
        } else if (!remote_wuji_proxy_) {
    ...
    bool GatherRobotStateToLogger() {
      ...
      // Robot state data is logged in policy/MuJoCo order. Keep an
      // absolute-position copy in the same order for replay safety limits.
      std::array<double, G1_NUM_MOTOR> body_q = {0.0};
      std::array<double, G1_NUM_MOTOR> body_q_absolute = {0.0};
      std::array<double, G1_NUM_MOTOR> body_dq = {0.0};
      const auto replay_info = GetReplayStreamInfo();
      const bool replay_armed =
          replay_info.has_packet || replay_info.faulted;
      const bool replay_safety_armed =
          replay_armed && !disable_replay_safety_;
      replay_safety_stop_requested_this_tick_ = false;
      replay_safety_reset_count_ = static_cast<std::int64_t>(
          replay_info.safety_reset_count);

      auto unitree_joint_state = ls->motor_state();
      for (int i = 0; i < G1_NUM_MOTOR; i++) {
        body_q_absolute[i] =
            unitree_joint_state[mujoco_to_isaaclab[i]].q();
        body_q[i] = body_q_absolute[i] -
                    default_angles[mujoco_to_isaaclab[i]];
        body_dq[i] = unitree_joint_state[mujoco_to_isaaclab[i]].dq();
        if (!replay_safety_armed && body_dq[i] > 35 &&
            !disable_crc_check_) {
          std::cout << "✗ Error: body_dq[" << i << "] = " << body_dq[i]
                    << " > 35." << std::endl;
          return false;
        }
      }
      ...
      std::array<double, 4> base_quat =
          float_to_double<4>(ls->imu_state().quaternion()); // qw, qx, qy, qz
      if (replay_safety_armed) {
        const auto safety =
            sonic_replay::ReplayPayloadSafety::EvaluateMeasuredState(
                body_q_absolute, body_dq, base_quat);
        const bool measured_state_safe =
            safety.finite && !safety.fall_detected &&
            !safety.joint_position_violation &&
            !safety.joint_velocity_violation;
        const auto epoch_update = replay_safety_epoch_gate_.Observe(
            replay_info, measured_state_safe);
        if (epoch_update.rearm_accepted) {
          // Per-epoch telemetry may clear only after the new explicit source
          // epoch and the robot's current measured state are both safe.
          replay_safety_fall_detected_ = false;
          replay_joint_position_violation_count_ = 0;
          replay_joint_velocity_violation_count_ = 0;
          replay_command_limit_violation_count_ = 0;
        }
        if (epoch_update.reset_count_blocks_run) {
          replay_safety_stop_requested_this_tick_ = true;
        }
        if (safety.fall_detected) {
          replay_safety_fall_detected_ = true;
        }
        if (safety.joint_position_violation) {
          ++replay_joint_position_violation_count_;
        }
        if (safety.joint_velocity_violation) {
          ++replay_joint_velocity_violation_count_;
        }
        if (!measured_state_safe) {
          replay_safety_stop_requested_this_tick_ = true;
        }
      }
      ...
      WujiHandJointArray left_hand_q = kDefaultWujiHandPose;
      WujiHandJointArray left_hand_dq = kDefaultWujiHandPose;
      WujiHandJointArray right_hand_q = kDefaultWujiHandPose;
      WujiHandJointArray right_hand_dq = kDefaultWujiHandPose;
      
      if (use_wuji_hands_) {
        left_hand_q = wuji_hands_.getJointActualPosition(true);
        right_hand_q = wuji_hands_.getJointActualPosition(false);
      } else if (!remote_wuji_proxy_) {
      ...
      if (state_logger_) {
        ...
        const auto tick_timestamp_monotonic =
        ...
        state_logger_->LogFullState(base_quat, base_ang_vel, base_accel, body_torso_quat, body_torso_ang_vel, body_torso_accel,
                                    ...
                                    ros_timestamp, tick_timestamp_monotonic);
      }
      return true;
    }
    ...
    bool GatherInputInterfaceData() {
      ...
      const HandPoseSnapshot hand_snapshot =
          input_interface_->GetHandPoseSnapshot();
      has_left_hand_data_ = hand_snapshot.left_valid;
      has_right_hand_data_ = hand_snapshot.right_valid;
      left_hand_joint_buffer_ = hand_snapshot.left;
      right_hand_joint_buffer_ = hand_snapshot.right;
      hand_frame_index_buffer_ = hand_snapshot.hand_frame_index;
      ...
      if (initial_encoder_mode_ != -2) {
        // Default deploy recomputes/clears token_state every control tick. The VLA
        // bridge target can hold a decoder-only bootstrap/external token instead.
        const bool hold_replay_encoder_token =
            is_using_encoder_ && GetReplayStreamInfo().enabled &&
            GetReplayStreamInfo().source_hz > 0.0;
#if ENABLE_VLA_SONIC_BRIDGE
        if (is_using_encoder_ && !hold_replay_encoder_token) {
          token_state_data_.assign(token_state_data_.size(), 0.0);
        }
#else
        if (!hold_replay_encoder_token) {
          token_state_data_.assign(token_state_data_.size(), 0.0);
        }
#endif
...
#if ENABLE_VLA_SONIC_BRIDGE
          if (has_decoder_only_token_state_) {
            return true;
          }
#endif
    ...
    bool CreatePolicyCommand() {
      ...
      const bool replay_armed =
          replay_info.has_packet || replay_info.faulted ||
          replay_info.safety_reset_count > 0;
      const bool replay_safety_armed =
          replay_armed && !disable_replay_safety_;
      bool command_limit_violation = false;
      if (replay_safety_armed) {
        for (int hardware_joint = 0; hardware_joint < G1_NUM_MOTOR;
             ++hardware_joint) {
          const std::size_t policy_joint = static_cast<std::size_t>(
              isaaclab_to_mujoco[hardware_joint]);
          const double policy_action =
              static_cast<double>(floatarr[policy_joint]);
          const double q_target = motor_command_tmp.q_target[hardware_joint];
          const double dq_target = motor_command_tmp.dq_target[hardware_joint];
          const double kp = motor_command_tmp.kp[hardware_joint];
          const double kd = motor_command_tmp.kd[hardware_joint];
          const double tau_ff = motor_command_tmp.tau_ff[hardware_joint];
          // q_target is a virtual PD setpoint, not a measured joint angle.
          // Validate it against the normalized-action envelope used in
          // training; mechanical limits apply to measured q.
          const double command_q_lower =
              default_angles[hardware_joint] -
              sonic_replay::kReplayPolicyActionAbsLimit *
                  g1_action_scale[hardware_joint];
          const double command_q_upper =
              default_angles[hardware_joint] +
              sonic_replay::kReplayPolicyActionAbsLimit *
                  g1_action_scale[hardware_joint];
          if (!sonic_replay::ReplayPayloadSafety::PolicyActionWithinEnvelope(
                  policy_action) ||
              !sonic_replay::ReplayIsFinite(q_target) ||
              q_target < command_q_lower -
                             sonic_replay::kReplayNumericLimitTolerance ||
              q_target > command_q_upper +
                             sonic_replay::kReplayNumericLimitTolerance ||
              !sonic_replay::ReplayIsFinite(dq_target) ||
              std::abs(dq_target) >
                  sonic_replay::kG1ReplayJointVelocityLimits[policy_joint] +
                      sonic_replay::kReplayNumericLimitTolerance ||
              !sonic_replay::ReplayIsFinite(kp) ||
              !sonic_replay::ReplayIsFinite(kd) ||
              !sonic_replay::ReplayIsFinite(tau_ff)) {
            command_limit_violation = true;
            break;
          }
        }
      }
      if (command_limit_violation) {
        ++replay_command_limit_violation_count_;
        replay_safety_stop_requested_this_tick_ = true;
      } else if (!replay_safety_stop_requested_this_tick_) {
        // Never expose an invalid policy command to the 500 Hz writer. The
        // logger still records it below so the same control tick proves why
        // replay stopped.
        motor_command_buffer_.SetData(motor_command_tmp);
      }
      if (state_logger_ && !state_logger_->LogPostMotorCommand(motor_command_tmp)) {
        std::cerr << "[WARNING] Failed to log RUN motor command to state logger"
                  << std::endl;
      }
      return true;
    }
    ...
    bool CurrentFrameAdvancement() {
      ...
      if (!use_planner_motion && (IsV1ReplayActive() || IsV4ReplayActive())) {
        // Replay packets are the source clock. Control ticks between packets
        // hold the cached replay state; they must not advance the legacy
        // motion cursor or trigger temporary-motion completion/heading reset.
        current_frame_ = 0;
        return true;
      }
     ...
     *    6. Update hand backend (Wuji default path, Dex3 legacy path retained).
      ...
      switch (program_state_) {
        ...
        case ProgramState::CONTROL: {
          ...
          std::shared_ptr<const MotionSequence> current_motion_copy = nullptr;
          {
            ...
            replay_encoder_cache_refresh_this_tick_ = false;
            ...
            current_encoder_mode_copy = current_motion_->GetEncodeMode();
          ...
          if (state_logger_) {
            ...
            const auto replay_info = GetReplayStreamInfo();
            std::int64_t replay_route = 0;
            if (replay_info.has_packet && replay_info.source_hz > 0.0) {
              replay_route = 1;
            }
#if ENABLE_VLA_SONIC_BRIDGE
            else if (!is_using_encoder_) {
              const auto [has_external_token, external_token] =
                  input_interface_->GetExternalTokenState();
              if (has_external_token && !external_token.empty()) {
                replay_route = 2;
              }
            }
#endif

            std::array<std::int64_t, 10> encoder_input_indices;
            encoder_input_indices.fill(-1);
            if (replay_route == 1) {
              for (std::size_t i = 0; i < encoder_input_indices.size(); ++i) {
                encoder_input_indices[i] = static_cast<std::int64_t>(
                    sonic_replay::StreamedReplayState::
                        ResolveObservationFrameOffset(
                            static_cast<int>(i), 5, replay_info.source_hz,
                            1.0 / control_dt_));
              }
            }
            ...
            if (!state_logger_->LogPostReplayMetadata(
                    replay_route,
                    static_cast<std::int64_t>(replay_info.stream_epoch),
                    replay_info.source_frame,
                    replay_encoder_cache_refresh_this_tick_,
                    encoder_input_indices,
                    last_pose_rx_age_ms,
                    replay_info.faulted ? 1 : 0,
                    replay_safety_fall_detected_,
                    replay_safety_reset_count_,
                    replay_joint_position_violation_count_,
                    replay_joint_velocity_violation_count_,
                    replay_command_limit_violation_count_,
                    hand_frame_index_buffer_)) {
              std::cerr << "[WARNING] Failed to log replay metadata to state logger"
                        << std::endl;
            }
          }

          if (!replay_safety_stop_requested_this_tick_ && use_wuji_hands_) {
            if (has_left_hand_data_) {
              wuji_hands_.setAllJointsCommand(true, left_hand_joint_buffer_);
            }
            if (has_right_hand_data_) {
              wuji_hands_.setAllJointsCommand(false, right_hand_joint_buffer_);
            }
          } else if (!replay_safety_stop_requested_this_tick_ &&
                     !remote_wuji_proxy_) {
            std::array<double, 7> dex3_left_command = {0.0};
            std::array<double, 7> dex3_right_command = {0.0};
            for (int i = 0; i < 7; ++i) {
              dex3_left_command[i] = left_hand_joint_buffer_[i];
              dex3_right_command[i] = right_hand_joint_buffer_[i];
            }
            ...
            dex3_hands_.setAllJointsCommand(true, dex3_left_command);
            dex3_hands_.setAllJointsCommand(false, dex3_right_command);
          }
          ...
          if (replay_safety_stop_requested_this_tick_) {
            std::cerr << "[SAFETY] Replay measured/command hard limit violated; "
                      << "stopping after safety telemetry publish." << std::endl;
            operator_state.stop = true;
            return;
          }
          ...
          if (logging_counter_ % 50 == 0) {
            ...
            if (remote_wuji_proxy_) {
              std::cout << " | HandBackend: remote_wuji_proxy";
            } else {
...
int main(int argc, char const* argv[]) {
  ...
  if (argc < 4) {
    ...
    std::cout << "  --v1-replay-source-hz <hz>: enable packet-clock protocol-v1 replay (supported values: 20 or 50)" << std::endl;
    std::cout << "  --replay-target <sim|real>: require every explicit replay pose packet to match the deploy target" << std::endl;
    std::cout << "  --disable-teleop-safety-gate: disable streamed teleop jump protection (default: enabled)" << std::endl;
    std::cout << "  --disable-replay-safety: disable replay measured-state and policy-command hard-stop checks" << std::endl;
    ...
    std::cout << "  --collector-zmq-out-port <port>: ZMQ port for collector output (default: 5558)" << std::endl;
    std::cout << "  --collector-zmq-out-topic <topic>: ZMQ topic/prefix for collector output (default: robot_state_action)" << std::endl;
    std::cout << "  --hand-backend <local_wuji|remote_wuji_proxy>: hand hardware backend (default: local_wuji)" << std::endl;
    std::cout << "  --left-wuji-serial <serial>: left WujiHand serial (default: 307735743434)" << std::endl;
    std::cout << "  --right-wuji-serial <serial>: right WujiHand serial (default: 3365364A3434)" << std::endl;
  ...
  bool zmq_verbose = false;
  double v1_replay_source_hz = 0.0;
  std::optional<bool> replay_target_real;
  bool disableTeleopSafetyGate = false;
  bool disableReplaySafety = false;
  ...
  for (int i = 4; i < argc; i++) {
    if (std::string(argv[i]) == "--disable-crc-check") {
    ...
    } else if (std::string(argv[i]) == "--collector-zmq-out-port") {
      if (i + 1 < argc) { collector_zmq_out_port = std::stoi(argv[i + 1]); i++; }
    } else if (std::string(argv[i]) == "--collector-zmq-out-topic") {
      if (i + 1 < argc) { collector_zmq_out_topic = argv[i + 1]; i++; }
      ...
      zmq_verbose = true;
    } else if (std::string(argv[i]) == "--replay-target") {
      if (i + 1 >= argc) {
        std::cerr << "Error: --replay-target requires sim or real" << std::endl;
        exit(1);
      }
      const std::string replay_target = argv[++i];
      if (replay_target == "sim") {
        replay_target_real = false;
      } else if (replay_target == "real") {
        replay_target_real = true;
      } else {
        std::cerr << "Error: --replay-target must be sim or real" << std::endl;
        exit(1);
      }
      std::cout << "[INFO] Explicit replay target: "
                << replay_target << std::endl;
    } else if (std::string(argv[i]) == "--v1-replay-source-hz") {
      if (i + 1 >= argc) {
        std::cerr << "Error: --v1-replay-source-hz requires a positive rate" << std::endl;
        exit(1);
      }
      try {
        v1_replay_source_hz = std::stod(argv[++i]);
      } catch (...) {
        std::cerr << "Error: invalid --v1-replay-source-hz value" << std::endl;
        exit(1);
      }
      if (!sonic_replay::StreamedReplayState::IsSupportedSourceRate(
              v1_replay_source_hz)) {
        std::cerr << "Error: --v1-replay-source-hz must be 20 or 50"
                  << std::endl;
        exit(1);
      }
      std::cout << "[INFO] Explicit v1 replay source rate: "
                << v1_replay_source_hz << " Hz" << std::endl;
    } else if (std::string(argv[i]) == "--disable-teleop-safety-gate") {
      disableTeleopSafetyGate = true;
      std::cout << "[INFO] Teleop safety gate disabled by CLI" << std::endl;
    } else if (std::string(argv[i]) == "--disable-replay-safety") {
      disableReplaySafety = true;
      std::cout << "[INFO] Replay measured/command safety disabled by CLI"
                << std::endl;
    ...
    } else if (std::string(argv[i]) == "--left-wuji-serial") {
      if (i + 1 < argc) {
        left_wuji_serial = argv[i + 1];
        std::cout << "[INFO] Using left WujiHand serial: " << left_wuji_serial << std::endl;
        i++;
      } else {
        std::cerr << "Error: --left-wuji-serial requires a value argument" << std::endl;
        exit(1);
      }
    } else if (std::string(argv[i]) == "--right-wuji-serial") {
      if (i + 1 < argc) {
        right_wuji_serial = argv[i + 1];
        std::cout << "[INFO] Using right WujiHand serial: " << right_wuji_serial << std::endl;
        i++;
      } else {
        std::cerr << "Error: --right-wuji-serial requires a value argument" << std::endl;
        exit(1);
      }
    } else if (std::string(argv[i]) == "--hand-backend") {
      if (i + 1 < argc) {
        hand_backend = argv[i + 1];
        if (hand_backend != "local_wuji" && hand_backend != "remote_wuji_proxy") {
          std::cerr << "Error: --hand-backend must be local_wuji or remote_wuji_proxy" << std::endl;
          exit(1);
        }
        std::cout << "[INFO] Using hand backend: " << hand_backend << std::endl;
        i++;
      } else {
        std::cerr << "Error: --hand-backend requires a value argument" << std::endl;
        exit(1);
      }
    }
  }

  if (replay_target_real.has_value()) {
    const bool network_input =
        inputType == "zmq" || inputType == "zmq_manager" ||
        inputType == "manager" || inputType == "gamepad_manager";
    if (!network_input) {
      std::cerr << "Error: --replay-target requires a ZMQ-capable input type"
                << std::endl;
      exit(1);
    }
  }

  if (v1_replay_source_hz > 0.0) {
    const bool network_input =
        inputType == "zmq" || inputType == "zmq_manager" ||
        inputType == "manager" || inputType == "gamepad_manager";
    if (!network_input) {
      std::cerr << "Error: --v1-replay-source-hz requires a ZMQ-capable input type"
                << std::endl;
      exit(1);
    }
    if (encoderFile.empty()) {
      std::cerr << "Error: --v1-replay-source-hz requires --encoder-file (local encoder mode)"
                << std::endl;
      exit(1);
    }
  }
  ...
  G1Deploy custom(
    ...
    zmq_out_port,
    zmq_out_topic,
    collector_zmq_out_port,
    collector_zmq_out_topic,
    enableMotionRecording,
    initial_compliance,
    initial_max_close_ratio,
    disableTeleopSafetyGate,
    left_wuji_serial,
    right_wuji_serial,
    hand_backend,
    v1_replay_source_hz,
