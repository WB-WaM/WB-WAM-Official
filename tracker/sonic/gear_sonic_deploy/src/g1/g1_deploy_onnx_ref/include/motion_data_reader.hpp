...
struct MotionSequence {
    ...
    /** Stream-source provenance is immutable to control/collector consumers. */
    bool HasSourceProvenance() const { return has_source_provenance_; }
    std::int64_t SourceWindowStart() const { return source_window_start_; }
    std::int64_t SourceFrameStep() const { return source_frame_step_; }
    std::int64_t SourceGenerationIndex() const { return source_generation_index_; }
    std::int64_t SourceFrameIndex(int local_frame) const {
      return has_source_provenance_
                 ? source_window_start_ +
                       source_frame_step_ * static_cast<std::int64_t>(local_frame)
                 : -1;
    }
    ...
    friend class StreamedMotionMerger;

    void SetSourceProvenance(
        std::int64_t window_start, std::int64_t frame_step,
        std::int64_t generation_index) {
      has_source_provenance_ = true;
      source_window_start_ = window_start;
      source_frame_step_ = frame_step;
      source_generation_index_ = generation_index;
    }

    bool has_source_provenance_ = false;
    std::int64_t source_window_start_ = -1;
    std::int64_t source_frame_step_ = 1;
    std::int64_t source_generation_index_ = -1;
