...
public:
     ...
     * @param left_hand_joint            Left-hand WujiHand joint positions (20 DOF).
     * @param right_hand_joint           Right-hand WujiHand joint positions (20 DOF).
    ...
    virtual void publish(
        ...
        const WujiHandJointArray& left_hand_joint,
        const WujiHandJointArray& right_hand_joint,
    ...
    /**
     * Replay-only terminal telemetry hooks. Other output transports keep the
     * default no-op behavior so a fault publish cannot trigger their full
     * state/motion packing path.
     */
    virtual bool GetLastPublishedStateEntry(
        StateLogger::Entry&) const noexcept {
        return false;
    }

    virtual bool PublishReplayEntry(
        const StateLogger::Entry&) noexcept {
        return false;
    }
...
protected:
     ...
     *   left_hand_q_measured   | 20   | Left-hand WujiHand joint positions.
     *   right_hand_q_measured  | 20   | Right-hand WujiHand joint positions.
    ...
    )
    {
        ...
        if (state.left_hand_q.size() == kWujiHandDoF) {
        ...
        if (state.right_hand_q.size() == kWujiHandDoF) {
