...
class InterfaceManager : public InputInterface {
  public:
    ...
    HandPoseSnapshot GetHandPoseSnapshot() const override {
      if (current_) {
        return current_->GetHandPoseSnapshot();
      }
    ...
    sonic_replay::ReplayStreamInfo GetReplayStreamInfo() const override {
      return current_ ? current_->GetReplayStreamInfo()
                      : InputInterface::GetReplayStreamInfo();
    }
  ...
  private:
    ...
    void buildInterfaces() {
      ...
      zmq_ = std::make_unique<ZMQEndpointInterface>(
        ...
        v1_replay_source_hz_, replay_target_real_
      );
