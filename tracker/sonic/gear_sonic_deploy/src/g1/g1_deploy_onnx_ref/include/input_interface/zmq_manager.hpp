  ...
  public:
    ...
    ZMQManager(
      ...
      pose_interface_ = std::make_unique<ZMQEndpointInterface>(
        ...
        v1_replay_source_hz, replay_target_real
      );
    ...
    void handle_input(MotionDataReader& motion_reader,
      ...
      if (active_mode_ == ManagedMode::PLANNER) {
        ...
        if (start_control_ && !operator_state.start) {
          ...
          std::cout << "[ZMQManager] Start control in STREAMED MOTION mode" << std::endl;
        }
    ...
    HandPoseSnapshot GetHandPoseSnapshot() const override {
      if ((active_mode_ == ManagedMode::STREAMED_MOTION || (!is_planner_ready_ && switch_from_teleop_to_planner_)) && pose_interface_) {
        return pose_interface_->GetHandPoseSnapshot();
      }
    ...
    sonic_replay::ReplayStreamInfo GetReplayStreamInfo() const override {
      if ((active_mode_ == ManagedMode::STREAMED_MOTION ||
           (!is_planner_ready_ && switch_from_teleop_to_planner_)) &&
          pose_interface_) {
        return pose_interface_->GetReplayStreamInfo();
      }
      return InputInterface::GetReplayStreamInfo();
    }
  ...
  private:
    ...
    void OnPlannerReceived(
      ...
      for (size_t i = 0; i < hdr.fields.size(); ++i) {
        ...
        else if (f.name == "left_wuji_qpos" || f.name == "left_hand_joints") left_hand_joints_idx = static_cast<int>(i);
        else if (f.name == "right_wuji_qpos" || f.name == "right_hand_joints") right_hand_joints_idx = static_cast<int>(i);
      ...
      // Optional: left-hand joint targets (Wuji 20 DOF, legacy Dex3 7 DOF)
      if (left_hand_joints_idx >= 0) {
        const auto& lh_buf = bufs[left_hand_joints_idx];
        const auto& lh_field = hdr.fields[left_hand_joints_idx];

        WujiHandJointArray left_hand_joints_data = kDefaultWujiHandPose;
        size_t total_elements = 1;
        for (auto dim : lh_field.shape) total_elements *= dim;
        size_t decode_count = total_elements == kWujiHandDoF ? kWujiHandDoF : std::min<std::size_t>(7, total_elements);
      ...
      // Optional: right-hand joint targets (Wuji 20 DOF, legacy Dex3 7 DOF)
      if (right_hand_joints_idx >= 0) {
        ...
        WujiHandJointArray right_hand_joints_data = kDefaultWujiHandPose;
        size_t total_elements = 1;
        for (auto dim : rh_field.shape) total_elements *= dim;
        size_t decode_count = total_elements == kWujiHandDoF ? kWujiHandDoF : std::min<std::size_t>(7, total_elements);
