...
class ElasticBand:
    ...
    def set_enabled(self, enabled: bool):
        """Set support explicitly; managed replay must never rely on toggles."""
        self.enable = bool(enabled)

    def set_anchor_position(self, position):
        """Anchor managed support at a finite world-space body position."""
        position = np.asarray(position, dtype=float)
        if position.shape != (3,) or not np.all(np.isfinite(position)):
            raise ValueError("ElasticBand anchor position must be finite shape (3,)")
        self.point = position.copy()
        self.length = 0

    def _manual_control_blocked(self, key):
        if self.managed:
            print(f"ElasticBand managed control active; ignored manual key {key}")
            return True
        return False
    ...
    def MujuocoKeyCallback(self, key):
        ...
        if key in (
            ...
            glfw.KEY_8,
            glfw.KEY_9,
        ) and self._manual_control_blocked(key):
            return
        if key == glfw.KEY_7:
            self.length -= 0.1
        if key == glfw.KEY_8:
            self.length += 0.1
        if key == glfw.KEY_9:
            self.set_enabled(not self.enable)

    def handle_keyboard_button(self, key):
        if key == "9":
            if self._manual_control_blocked(key):
                return
            self.set_enabled(not self.enable)
